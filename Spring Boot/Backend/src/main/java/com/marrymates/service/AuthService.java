...
//    }
    ...
    public void sendPasswordResetLink(String email) {
        ...
        if (existingTokenOpt.isPresent()) {
            // Update the existing token: change its token value and expiry date
            resetToken = existingTokenOpt.get();
            resetToken.setToken(token);
            resetToken.setExpiryDate(calculateExpiryDate(24 * 60)); // 24 hours from now
        } else {
            // Create a new token if none exists for this user
